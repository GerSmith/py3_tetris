...
"""Тетрис. Версия написанная на pygame"""
...
# Блок основных констант
fps = 25  # кадровая частота
window_w, window_h = 600, 500  # высота и ширина окна программы
# размер базового элемента фигур-букв block, размер стакана
block, cup_h, cup_w = 20, 20, 10
# время, которое затрачивается на перемещение фигуры
side_freq, down_freq = 0.15, 0.1

# дистанция между правой и левой сторонами окна программы и стаканом
side_margin = int((window_w - cup_w * block) / 2)
# расстояние между верхней границей стакана и окном
...
# Описание шаблона
fig_w, fig_h = 5, 5  # размер
empty = "o"  # пустой элемент

# Фигуры и все возможные варианты поворотов
figures = {
    "S": [
    ...
    while True:
        ...
        for event in pg.event.get():
            if event.type == KEYUP:
                ...
                elif event.key == K_RIGHT and checkPos(
                ...
                # поворачиваем фигуру, если есть место
                    ...
                    fallingFig["rotation"] = (
                        fallingFig["rotation"] + 1
                    ) % len(figures[fallingFig["shape"]])
                    if not checkPos(cup, fallingFig):
                        fallingFig["rotation"] = (
                            fallingFig["rotation"] - 1
                        ) % len(figures[fallingFig["shape"]])
        ...
        if (
            going_left or going_right
        ) and time.time() - last_side_move > side_freq:
        ...
        if (
            going_down
            and time.time() - last_move_down > down_freq
            and checkPos(cup, fallingFig, adjY=1)
        ):
            fallingFig["y"] += 1
            last_move_down = time.time()
...
def emptycup():
    """Функция рисования пустого стакана."""
    cup = []
...
def checkPos(cup, fig, adjX=0, adjY=0):
    # проверяет, находится ли фигура в границах стакана,
    # не сталкиваясь с другими фигурами
    for x in range(fig_w):
        for y in range(fig_h):
            abovecup = y + fig["y"] + adjY < 0
            if (
                abovecup
                or figures[fig["shape"]][fig["rotation"]][y][x] == empty
            ):
                continue
...
def drawTitle():
    ...
    titleRect.topleft = (window_w - 390, 30)
    display_surf.blit(titleSurf, titleRect)
